...
import { onAuthChange, ensureAdminProfile, getUserProfile, logOut, isAdmin, getCurrentUser, signIn, resetPassword } from '../../src/firebase/auth.js';
...
.card {
  ...
  box-shadow:
    0 30px 70px rgba(0, 0, 20, 0.55),
    0 10px 30px rgba(0, 0, 20, 0.3),
...
.card::before {
  ...
  background: linear-gradient(90deg,
    var(--saudi-green) 0%, var(--saudi-green-light) 40%,
    var(--gold) 60%, var(--gold-light) 100%);
}

.card-title { font-family: 'Cairo', sans-serif; font-size: 17px; font-weight: 900; color: var(--navy); margin-bottom: 3px; }
.card-subtitle { font-size: 11px; color: rgba(10,24,40,0.55); font-weight: 600; margin-bottom: 16px; }

.field { margin-bottom: 10px; position: relative; }
.field .input-icon {
  position: absolute; top: 50%; right: 13px; transform: translateY(-50%);
  color: var(--gold-dark); font-size: 15px; pointer-events: none;
}
.field input {
  width: 100%;
...
.field input::placeholder { color: rgba(10,24,40,0.4); }
.field input:focus {
...
.login-btn {
  ...
  background: linear-gradient(135deg,
    var(--saudi-green-dark) 0%, var(--saudi-green) 50%, var(--saudi-green-light) 100%);
  color: white;
  border: none;
  border-radius: 10px;
  font-size: 15px;
  ...
  box-shadow: 0 8px 20px rgba(0, 108, 53, 0.45), inset 0 1px 0 rgba(255,255,255,0.15);
  border-top: 1px solid var(--gold);
  transition: all 0.25s;
}
.login-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 26px rgba(0, 108, 53, 0.55); }
...
.forgot-link:hover { color: var(--saudi-green-dark); text-decoration: underline; }

/* ═══════════════ شريط الصور العلوي (طائرة/سفينة/شاحنة/طريق) ═══════════════ */
...
.hero-band-shade {
  ...
  background: linear-gradient(180deg,
    rgba(8, 16, 28, 0.55) 0%,
    rgba(8, 16, 28, 0.05) 45%,
    rgba(8, 16, 28, 0.10) 60%,
    rgba(8, 16, 28, 0.35) 100%);
}
/* الشريط الأخير — أطول لاستيعاب صف طرق الشحن وبطاقة الدخول فوق الصورة مباشرة */
.hero-band--gate {
  height: 34vh;
  min-height: 320px;
  max-height: 380px;
}
.hero-band--gate .hero-band-shade {
  background: linear-gradient(180deg,
    rgba(8, 16, 28, 0.28) 0%,
    rgba(8, 16, 28, 0.04) 30%,
    rgba(8, 16, 28, 0.18) 62%,
    rgba(8, 16, 28, 0.58) 100%);
}
...
/* صف طرق الشحن (جوًا/بحرًا/برًا) + الشعار — أسفل يسار الشريط الأخير، بجانب البطاقة */
.hero-modes {
  position: absolute; left: 22px; bottom: 26px;
  z-index: 3;
  max-width: calc(50% - 22px);
}
.hero-modes-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
.hero-mode { display: flex; align-items: center; gap: 5px; }
.hero-mode span {
  color: #fff; font-size: 11px; font-weight: 800;
  text-shadow: 0 1px 8px rgba(0, 0, 0, 0.55);
}
.hero-mode svg { width: 14px; height: 14px; color: var(--gold-light); }
.hero-mode-sep { width: 20px; height: 1px; background: rgba(255, 255, 255, 0.4); }
.hero-tagline {
  color: #fff; font-size: 11px; font-weight: 800;
  text-shadow: 0 1px 8px rgba(0, 0, 0, 0.55);
}
...
@media (max-width: 720px) {
  .hero-band { height: 12vh; min-height: 66px; max-height: 92px; }
  .hero-band--gate { height: auto; min-height: 0; max-height: none; }
  .hero-band--gate .hero-band-shade { display: none; }
  .hero-band--gate { background-image: none !important; background-color: var(--cream); }
  .hero-topbar { top: 10px; right: 14px; max-width: 82%; gap: 6px; }
  .hero-logo-badge { width: 30px; height: 30px; border-radius: 7px; }
  .hero-modes { position: static; max-width: none; padding: 16px 16px 4px; }
  .hero-modes-row { justify-content: center; }
  .hero-tagline { text-align: center; display: block; color: var(--saudi-green-dark); text-shadow: none; }
  .hero-mode span { color: var(--navy); text-shadow: none; }
  .hero-mode svg { color: var(--gold-dark); }
  .hero-mode-sep { background: rgba(10,24,40,0.15); }
  .wrap { position: static; width: calc(100% - 32px); max-width: 380px; margin: 0 auto; padding-bottom: 22px; bottom: auto; right: auto; }
}
</style>
...
<div class="hero">
  ...
  <!-- الشريط الأخير: طريق شحن + صف طرق النقل + بطاقة تسجيل الدخول فوق الصورة مباشرة -->
  <div class="hero-band hero-band--gate" style="background-image:url('https://images.pexels.com/photos/2768483/pexels-photo-2768483.jpeg?auto=compress&amp;cs=tinysrgb&amp;w=1600')">
    <div class="hero-band-shade"></div>

    <div class="hero-modes">
      <div class="hero-modes-row">
        <div class="hero-mode">
      ...
      <div class="hero-tagline">السديس اللوجستية .. على كل طريق</div>
    </div>

    <div class="wrap">
      <div class="card">
        <div class="card-title">تسجيل الدخول</div>
        <div class="card-subtitle">أدخل بريدك وكلمة المرور للوصول إلى حسابك</div>
        ...
        <div id="err" style="display:none;padding:10px;margin-bottom:10px;background:rgba(196,24,24,0.1);color:#c41818;border:1px solid rgba(196,24,24,0.3);border-radius:6px;font-size:12px;align-items:center;gap:8px;"></div>
        ...
        <div class="forgot-link" onclick="doForgotPassword()">نسيت كلمة المرور؟</div>
      </div>
    </div>
  </div>
</div>
...
`;
  ...
  window.doForgotPassword = async () => {
    ...
    const email = document.getElementById('email').value.trim();
    if (!email) {
      errBox.innerHTML = '<i class="ti ti-alert-circle"></i> اكتب بريدك الإلكتروني أولاً في الحقل أعلاه';
      errBox.style.background = 'rgba(196,24,24,0.1)';
      errBox.style.color = '#c41818';
      errBox.style.borderColor = 'rgba(196,24,24,0.3)';
      errBox.style.display = 'flex';
      return;
    }
    try {
      await resetPassword(email);
      errBox.innerHTML = '<i class="ti ti-mail-check"></i> تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك';
      errBox.style.background = 'rgba(0,108,53,0.1)';
      errBox.style.color = 'var(--saudi-green-dark)';
      errBox.style.borderColor = 'rgba(0,108,53,0.3)';
      errBox.style.display = 'flex';
    } catch (e) {
      const msgs = {
      ...
      errBox.innerHTML = `<i class="ti ti-alert-circle"></i> ${msgs[e.code] || 'تعذّر إرسال رابط إعادة التعيين'}`;
      errBox.style.background = 'rgba(196,24,24,0.1)';
      errBox.style.color = '#c41818';
      errBox.style.borderColor = 'rgba(196,24,24,0.3)';
      errBox.style.display = 'flex';
