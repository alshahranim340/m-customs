    ...
    <div class="ship-item">
      ...
      <div class="ship-actions">
        ${STATUS[s.status]?.next
          ? `<button class="btn btn-sm btn-primary" onclick="advanceStatus('${s.id}','${STATUS[s.status].next}')">${STATUS[s.status].nextAr}</button>`
          : ''}
...
async function openEditModal(id) {
  ...
  document.getElementById('edit-form-body').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
      <div class="field"><label>الوجهة</label><select id="e-dest" onchange="updatePort()">${destOpts}</select></div>
      <div class="field"><label>المنفذ</label><input type="text" id="e-port" value="${PORTS[s.port]||PORTS.uae}"></div>
  ...
  window.updatePort = () => {
    ...
    document.getElementById('e-port').value = PORTS[d] || PORTS.uae;
  };
}
